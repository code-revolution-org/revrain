...
- 各页面导航机制顺序一致，正文排版统一（WCAG 3.2.3 / 3.2.4）
- 引用块内边距由自身承担、内部段落不带外边距，用户覆盖文本间距时不错位（WCAG 1.4.12）

## 正文排版

所有页面模板均以 `<div class="markdown">{{ .Content }}</div>` 包裹正文，排版规则集中在 `assets/scss/base/_markdown.scss`（唯一来源）。如需自定义正文样式，请扩展 `.markdown`，勿为单个页面另写一套。
...
欢迎报告问题、改进文档或提交代码。请阅读 [CONTRIBUTING.md](CONTRIBUTING.md) 了解贡献方式。

## 更新日志

记录项目每个版本的重要变更。请查看 [CHANGELOG.md](CHANGELOG.md)。
