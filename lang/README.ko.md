...
- 페이지 간 내비게이션 순서가 일관되고 본문 타이포그래피가 통일됨 (WCAG 3.2.3 / 3.2.4)
- 인용 블록의 패딩은 블록 자체가 담당하고 내부 단락에는 마진이 없어, 텍스트 간격을 재정의해도 레이아웃이 깨지지 않음 (WCAG 1.4.12)

## 본문 타이포그래피

모든 페이지 템플릿은 본문을 `<div class="markdown">{{ .Content }}</div>`로 감쌉니다. 타이포그래피 규칙은 `assets/scss/base/_markdown.scss` 한 곳에 모여 있습니다. 본문 스타일을 사용자 지정하려면 `.markdown`을 확장하고 개별 페이지용으로 따로 작성하지 마세요.
