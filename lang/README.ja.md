...
- 各ページでナビゲーションの順序が一致し、本文タイポグラフィも統一（WCAG 3.2.3 / 3.2.4）
- 引用ブロックのパディングは自身が持ち、内部の段落にマージンがないため、テキスト間隔を上書きしても崩れない（WCAG 1.4.12）

## 本文タイポグラフィ

すべてのページテンプレートは本文を `<div class="markdown">{{ .Content }}</div>` で包みます。タイポグラフィ規則は `assets/scss/base/_markdown.scss` に一元化されています。本文スタイルをカスタマイズする場合は `.markdown` を拡張し、個別ページ用に別途定義しないでください。
