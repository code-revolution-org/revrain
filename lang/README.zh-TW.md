...
- 各頁面導覽機制順序一致，正文排版統一（WCAG 3.2.3 / 3.2.4）
- 引用區塊內距由自身承擔、內部段落不帶外距，使用者覆寫文字間距時不錯位（WCAG 1.4.12）

## 正文排版

所有頁面模板均以 `<div class="markdown">{{ .Content }}</div>` 包裹正文，排版規則集中於 `assets/scss/base/_markdown.scss`（唯一來源）。如需自訂正文樣式，請擴充 `.markdown`，勿為單一頁面另寫一套。
