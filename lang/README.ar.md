...
avatar_alt = "وصف الأفاتار"  # نص بديل للأفاتار (اختياري، فارغ = زخرفي، إمكانية وصول)
...
تستخدم صور المقالات leaf bundle (حزمة صفحة): ضع الصور في مجلد يحمل اسم المقال، ثم أشر إليها في Markdown باسم الملف مباشرة.

```
content/posts/my-post/
├── index.md        # نص المقال
└── photo.jpg       # مورد الصفحة (صورة)
```

الإشارة في Markdown (يعالجها خطاف عرض الصور، ويُخرج تلقائيًا `srcset` متجاوبًا و`width`/`height` أصليين و`loading="lazy"`):

```markdown
![نص بديل](photo.jpg "نص العنوان")
```

- الصورة المستقلة (وحدها في سطر) تُعرض كـ `<figure>` + `<figcaption>` (نص العنوان يصبح التعليق)
- الصورة داخل السطر تُعرض كـ `<img>`
- الصيغ القابلة للمعالجة (JPEG/PNG/WebP/AVIF وغيرها) تُنشئ `srcset` متعدد المستويات بـ WebP؛ أما SVG/ICO فتُخرج كما هي (بدون معالجة وبدون أبعاد أصلية)

يمكنك أيضًا استخدام الكود المختصر `figure`:

```markdown
{{< figure src="photo.jpg" alt="نص بديل" caption="العنوان" >}}
```

تُحلّ صور المشاركة الاجتماعية (og:image / twitter:image) بهذا الترتيب: معامل `images` للصفحة → مورد الغلاف (`*feature*`/`*cover*`/`*thumbnail*`) → `params.images` للموقع.
...
- ترتيب تنقل متسق وتنسيق نصي موحّد للجسم عبر الصفحات (WCAG 3.2.3 / 3.2.4)
- الحشو الداخلي للاقتباس يحمله العنصر نفسه؛ الفقرات الداخلية بلا هوامش، فلا يتشوّه المحتوى عند تجاوز تباعد النص (WCAG 1.4.12)

## تنسيق النص

كل قالب صفحة يغلّف محتواه بـ `<div class="markdown">{{ .Content }}</div>`. قواعد التنسيق في مكان واحد: `assets/scss/base/_markdown.scss`. لتخصيص الأنماط، وسّع `.markdown` بدلاً من كتابة مجموعة منفصلة لصفحة واحدة.
