...
[params.favicons]             # Иконки вкладки браузера (все необязательно)
...
**Цепочка поиска фавиконок:** каждая запись разрешается как *путь конфигурации → имя файла по соглашению в `static/` → пропуск*. Если ни одна иконка не настроена и не существует, выводится пустой URI `data:`, чтобы подавить стандартный запрос браузера к `/favicon.ico` (без 404). Пути могут быть внешними URL (`https://...`) или относительными путями сайта.
