...
- **내비게이션 일관성**: 찾아보기/분류/정보 페이지에 뒤로 가기 버튼과 이동 경로를 추가. `breadcrumb.html`은 `.Ancestors`가 비어 있는 경우(분류 개요 페이지) 가드와 빈 제목 대체 값을 보강
- **정보 페이지**: `.about__content`를 제거하고 공통 `.markdown` 레이어로 전환(호환성 깨짐)
